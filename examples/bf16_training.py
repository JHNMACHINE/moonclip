...
# ─── bf16 casting helpers ────────────────────────────────────────────

def cast_state_dict(sd: dict, dtype: torch.dtype) -> dict:
    """Cast all float tensors in a state_dict to the given dtype."""
    out = {}
    for k, v in sd.items():
        if isinstance(v, torch.Tensor) and v.is_floating_point():
            out[k] = v.to(dtype)
        elif isinstance(v, dict):
            out[k] = cast_state_dict(v, dtype)
        else:
            out[k] = v
    return out
...
class Bf16Wrapper:
    """
    Wraps a module or optimizer to save in bf16 and load back to fp32.
    Used by CheckpointManager: save() casts to bf16, load() casts back.
    """

    def __init__(self, obj, save_dtype=torch.bfloat16, compute_dtype=torch.float32):
        self._obj = obj
        self._save_dtype = save_dtype
        self._compute_dtype = compute_dtype

    def state_dict(self):
        sd = self._obj.state_dict()
        return cast_state_dict(sd, self._save_dtype)

    def load_state_dict(self, sd):
        sd_fp = cast_state_dict(sd, self._compute_dtype)
        self._obj.load_state_dict(sd_fp)
...
def train(
    ...
    use_bf16_ckpt = save_dtype in ("bf16", "bfloat16")
    ...
    # Wrappers for bf16 save/load
    def save_model():
        if use_bf16_ckpt:
            return Bf16Wrapper(model, torch.bfloat16, torch.float32)
        return model

    def save_optimizer():
        if use_bf16_ckpt:
            return Bf16Wrapper(optimizer, torch.bfloat16, torch.float32)
        return optimizer
    ...
    # Auto-resume: CheckpointManager handles it
    if fresh_start:
        print("Fresh start requested, ignoring existing checkpoints")
        start_step = 0
    else:
        start_step = ckpt.resume(
            model=load_model(), optimizer=load_optimizer(), scheduler=scheduler,
        )
        if start_step > 0:
            # Auto-detect save_dtype from resumed checkpoint metadata
            saved_dtype = ckpt.last_resume_metadata.get("save_dtype", "fp32")
            if saved_dtype == "bf16":
                use_bf16_ckpt = True
            elif saved_dtype == "fp32":
                use_bf16_ckpt = False
            print(f"Resumed from step {start_step} (dtype={saved_dtype})")
        else:
    ...
    dtype_label = "bf16" if use_bf16_ckpt else "fp32"
    print(f"\n{'='*70}")
    print(f"Training MiniGPT | {n_epochs} epochs | device={device} | ckpt dtype={dtype_label}")
    ...
    snap_id = ckpt.save(
        ...
        optimizer=save_optimizer(),
        scheduler=scheduler,
        metadata={"loss": f"{avg_loss:.4f}", "final": "true", "save_dtype": dtype_label},
    )
    ckpt.merge_now()
    ...
    print(f"\n{'='*70}")
    print(f"Training complete in {total_time:.1f}s | dtype={dtype_label}")
    ...
    if use_bf16_ckpt:
        ckpt2.load_latest(model=Bf16Wrapper(model2, torch.bfloat16, torch.float32))
    else:
    ...
    for (n1, p1), (n2, p2) in zip(model.named_parameters(), model2.named_parameters()):
        ...
        if use_bf16_ckpt:
            # bf16 has ~0.01 relative error for small weights
            if not torch.allclose(p1.data, p2.data, atol=0.01, rtol=0.01):
